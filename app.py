...
# --- 1. DATABASE SETUP ---
# Using v9_final to maintain consistency with the many-to-many mapping structure
...
if choice == "Settings":
    ...
    st.divider()
    st.subheader("🗑️ Delete Performance Data")
    st.info("Select a channel and date to remove records from the dashboard.")
    
    del_col1, del_col2 = st.columns(2)
    with del_col1:
        avail_channels = [r[0] for r in c.execute("SELECT name FROM channels").fetchall()]
        target_del_ch = st.selectbox("Select Channel to Clear", ["Select"] + avail_channels)
    with del_col2:
        target_del_date = st.date_input("Select Date to Clear", value=None)

    if st.button("Confirm Delete"):
        if target_del_ch != "Select" and target_del_date is not None:
            date_str = target_del_date.strftime('%Y-%m-%d')
            c.execute("DELETE FROM performance WHERE channel = ? AND date = ?", (target_del_ch, date_str))
            conn.commit()
            st.warning(f"Deleted all records for {target_del_ch} on {date_str}.")
        else:
            st.error("Please select both a Channel and a Date.")
